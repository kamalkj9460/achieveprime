  function ownKeys(object, enumerableOnly) {var keys = Object.keys(object);if (Object.getOwnPropertySymbols) {var symbols = Object.getOwnPropertySymbols(object);enumerableOnly && (symbols = symbols.filter(function (sym) {return Object.getOwnPropertyDescriptor(object, sym).enumerable;})), keys.push.apply(keys, symbols);}return keys;}function _objectSpread(target) {for (var i = 1; i < arguments.length; i++) {var source = null != arguments[i] ? arguments[i] : {};i % 2 ? ownKeys(Object(source), !0).forEach(function (key) {_defineProperty(target, key, source[key]);}) : Object.getOwnPropertyDescriptors ? Object.defineProperties(target, Object.getOwnPropertyDescriptors(source)) : ownKeys(Object(source)).forEach(function (key) {Object.defineProperty(target, key, Object.getOwnPropertyDescriptor(source, key));});}return target;}function _defineProperty(obj, key, value) {if (key in obj) {Object.defineProperty(obj, key, { value: value, enumerable: true, configurable: true, writable: true });} else {obj[key] = value;}return obj;}(function ($) {
        ...
        bar: {
          ...
          hideAnnouncement: (hide) => {
            if (hide) {
              ...
              $('.ly-switcher-wrapper').addClass('fix_lang');
            } else {
              $navBar.removeClass('announcement-hidden');
              $('.ly-switcher-wrapper').removeClass('fix_lang');
            }
          },
    ...
    this.scrollToMedia = function (mediaId) {
      ...
      if ($variantImage.length && $('body').hasClass('template-product') && theme.viewport.isSm()) {
        ...
        */
        $('.swiper-wrapper .swiper-slide').removeClass('current-img');
        $variantImage.parent().addClass('current-img');
      }
    };
    ...
    function handleModalThumbnailClick(e){
      	...
      	let curID = $(this).closest('[data-slick-index]').data('slick-index');
      	$('.product-image-zoom img').removeClass('active');
        $('.zoom-image').each(function(i){
          if(i==curID){
            $(this).addClass('active');
            $('.theme-images .theme-img').removeClass('current-img');
            $('.theme-img-'+(i+1)).addClass('current-img');
          }
        });

      	$('.product-area').find('.thumb-active').removeClass('thumb-active');
        $(this).addClass('thumb-active');
	...
	function handleImageClick() {
      if (theme.viewport.isSm()) {
        ...
        $('.ly-breakpoint-1, .ly-breakpoint-2, .cc-announcement').addClass('none');
		...
		let modalNav = $('.product-area__details__thumbs');
        modalNav.on('click', '.product-area__details__thumbs__thumb a', handleModalThumbnailClick);
        showThemeModal($('<div class="theme-modal product-image-zoom temp -light"  role="dialog" aria-modal="true"/>').append('<div class="product-area"><div class="product-area__details"></div></div>').append("\n           <div class='inner-scroller -out'>".concat(imageHtml, "</div>")), 'product-image', function ($modal) {
        ...
        $('.theme-modal .product-area__details').append(modalNav);
        $('.theme-modal #zoom-image').addClass('active');
      }
    } // Bind listeners
    ...
    theme.ProductRelated = new function () {
      let productRelatedSection = document.querySelector(".product-related");

      if (productRelatedSection === null) {
        return;
      } // Read product id from data attribute

      let swiperCont = $('.product-related .swiper-container');
      if (swiperCont.length === 1) {
          theme.initProductSlider(swiperCont);
      }
    }();
    ...
    jQuery(function ($) {
     ...
     var removeModalTimeoutID = -1;
      ...
      function closeThumbnailModal(e){
        e.preventDefault();
        let gallery = $(this).closest('.product-area__details__thumbs');
        let mediaId = $(this).closest('[data-media-id]').data('media-id');
        let media = gallery.find(".product-media[data-media-id=\"".concat(mediaId, "\"]")); //Scroll to that variant image
        ...
        //if (media.length) {
          ...
          let variantImage = $("[data-media-id=\"".concat(mediaId, "\"]"));
      ...
      window.closeThemeModal = function (immediate) {
        ...
        if($modal.find('.product-area').length==1){
          let prdNav = $modal.find('.product-area__details__thumbs');
          prdNav.on('click', '.product-area__details__thumbs__thumb a', closeThumbnailModal);
          $('.product-area__details__inner').append(prdNav);
          $('.ly-breakpoint-1, .ly-breakpoint-2, .cc-announcement').removeClass('none');
        }
      ...
      theme.Sections.register('product-related', theme.ProductRelated, {
